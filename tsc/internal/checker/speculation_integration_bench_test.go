...
func BenchmarkSpeculationWorkload(b *testing.B) {
	for _, tc := range []struct {
		...
		{"FirstOf8", 8, 0, 0},
		{"LastOf8", 8, 7, 0},
		{"Errors100LastOf8", 8, 7, 100},
		{"Errors1000LastOf8", 8, 7, 1000},
	} {
...
func BenchmarkSpeculationPermanentDiagnostics(b *testing.B) {
	for _, tc := range []struct {
		name                   string
		overloads, tag, errors int
	}{
		{"Circular100", 8, 7, 100}, {"Circular1000", 8, 7, 1000},
	} {
		b.Run(tc.name, func(b *testing.B) {
			var source strings.Builder
			for i := 0; i < tc.errors; i++ {
				fmt.Fprintf(&source, "type Circular%d = Circular%d;\n", i, i)
			}
			for i := 0; i < tc.overloads; i++ {
				fmt.Fprintf(&source, "declare function select<T>(value: T, fn: (x: T) => T, tag: %d): T;\n", i)
			}
