...
func (c *Checker) inferFromObjectTypes(n *InferenceState, source *Type, target *Type) {
	...
	if c.isArrayOrTupleType(source) {
		if isTupleType(target) {
			...
			if isTupleType(source) && c.hasCompatibleTupleStructure(source, target, TupleStructureComparisonContextForInference) {
...
type TupleStructureComparisonContext uint32

const (
	// Inference treats required and optional elements alike because both are inferred positionally.
	TupleStructureComparisonContextForInference TupleStructureComparisonContext = iota
	// Parameter projection treats rest and variadic elements alike because both occupy a variable argument range.
	TupleStructureComparisonContextForParameterProjection
)

func (c *Checker) hasCompatibleTupleStructure(t1 *Type, t2 *Type, context TupleStructureComparisonContext) bool {
	...
	for i, e1 := range t1.TargetTupleType().elementInfos {
		...
		if f1 != f2 &&
			!(context == TupleStructureComparisonContextForInference && f1&ElementFlagsFixed != 0 && f2&ElementFlagsFixed != 0) &&
			!(context == TupleStructureComparisonContextForParameterProjection && f1&ElementFlagsVariable != 0 && f2&ElementFlagsVariable != 0) {
			return false
		}
	}
	return true
